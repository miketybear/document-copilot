...
# Shared by the multi-turn tests below: a thread already one exchange deep, with a new
# question just sent as the latest message.
_LATER_TURN_REQUEST = ChatStreamRequest(
    id="thread-1",
    messages=[
...
async def _run_turn(
    monkeypatch,
    agent_result: FakeAgentResult | AsyncMock,
    *,
    request: ChatStreamRequest | None = None,
    generate_title: AsyncMock | None = None,
) -> RunTurnResult:
    """Patches every collaborator run_turn touches with a working default, then runs one turn.
    A test that needs to assert on (or fail) one particular collaborator grabs it off the
    returned RunTurnResult instead of re-declaring the whole patch set."""
    ...
    append_citations = AsyncMock()
    set_thread_title = AsyncMock()
    generate_title = generate_title or AsyncMock(return_value="Do X")
    log_info = Mock(wraps=orchestrator.logger.info)
    ...
    chunks = [chunk async for chunk in orchestrator.run_turn(USER, request or _request("How do I do X?"))]
    return RunTurnResult(chunks, append_message, append_citations, set_thread_title, generate_title, log_info)
...
async def test_valid_citation_streams_answer_and_persists_citations(monkeypatch):
    ...
    turn = await _run_turn(monkeypatch, result)
    ...
    assert turn.append_message.call_count == 2  # user message, then assistant message
    assistant_content = turn.append_message.await_args_list[1].args[3]
    assert any(part["type"] == "data-citation" for part in assistant_content["parts"])
    turn.append_citations.assert_awaited_once_with(
...
async def test_fabricated_citation_streams_error_and_does_not_persist_assistant_message(monkeypatch):
    ...
    turn.append_message.assert_called_once()  # only the user message was persisted, not the assistant reply
    turn.append_citations.assert_not_awaited()
...
async def test_fabricated_citation_is_retried_and_recovers(monkeypatch):
    ...
    turn = await _run_turn(monkeypatch, agent_run)
...
async def test_fabricated_citation_still_bad_after_retry_streams_error(monkeypatch):
    ...
    turn.append_message.assert_called_once()
    turn.append_citations.assert_not_awaited()
...
async def test_inline_citation_marker_is_stripped_before_streaming_and_persisting(monkeypatch):
    ...
    assert "[citation]" not in turn.stream

    persisted_assistant_content = turn.append_message.call_args_list[1].args[3]
...
async def test_no_citations_with_empty_retrieval_streams_answer(monkeypatch):
    ...
    assert "evidence" in turn.stream
    # append_citations is always called; it's the real function's job to no-op on an empty list.
    turn.append_citations.assert_awaited_once_with(USER, "assistant-msg", [])
...
async def test_first_turn_sets_thread_title_from_generated_title(monkeypatch):
    ...
    turn = await _run_turn(monkeypatch, result, generate_title=AsyncMock(return_value="Doing X"))

    turn.generate_title.assert_awaited_once_with("How do I do X?")
    turn.set_thread_title.assert_awaited_once_with(USER, "thread-1", "Doing X")
...
async def test_first_turn_falls_back_to_derived_title_when_generation_fails(monkeypatch):
    ...
    turn = await _run_turn(
        monkeypatch, result, generate_title=AsyncMock(side_effect=RuntimeError("model unavailable"))
    )

    turn.set_thread_title.assert_awaited_once_with(USER, "thread-1", "Do X")
...
async def test_prior_turns_are_passed_as_agent_message_history(monkeypatch):
    ...
    await _run_turn(monkeypatch, agent_run, request=_LATER_TURN_REQUEST)
...
async def test_logs_history_and_retrieval_usage(monkeypatch):
    ...
    turn = await _run_turn(monkeypatch, result, request=_LATER_TURN_REQUEST)

    turn.log_info.assert_any_call(
        "chat.history_built", thread_id="thread-1", prior_messages=2, kept_messages=2, kept_tokens=10
    )
    turn.log_info.assert_any_call("chat.retrieval_usage", thread_id="thread-1", retrieved_count=2, cited_count=1)
...
async def test_later_turn_does_not_touch_thread_title(monkeypatch):
    ...
    turn.set_thread_title.assert_not_awaited()
