...
- [ ] Frontend: Supabase email sign-in/sign-up qua `@supabase/supabase-js`, lưu session
- [ ] `backend/app/auth/dependencies.py` — verify `Authorization: Bearer`, expose `get_current_user`
- [ ] Frontend: route bảo vệ (redirect nếu chưa đăng nhập)
- [ ] Test thủ công: đăng nhập → gọi 1 endpoint backend cần auth → nhận đúng `user_id`
